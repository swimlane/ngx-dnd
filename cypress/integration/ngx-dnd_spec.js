...
describe('Demo', () => {
  ...
  const dragAndDrop = (fromElement, toElement, location = 'bottomRight') => {
    return cy.get(fromElement).should('be.visible').first().trigger('mousedown', { which: 1 })
      .get(toElement).should('be.visible').trigger('mousemove', location).trigger('mouseup');
  };
